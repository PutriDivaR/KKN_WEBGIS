...
@section('title', 'Daftar Rumah Adat — Admin Kampung Adat')
			...
			<form method="GET" action="{{ route('admin.rumah.index') }}" class="grid grid-cols-1 xl:grid-cols-[1fr_auto_auto_auto] gap-3">
				...
				<select name="suku" class="h-11 rounded-xl border border-[#d7e1d6] bg-white px-4 text-sm text-[#132018] outline-none">
					@foreach ($sukuList as $suku)
						<option value="{{ $suku }}" @selected($filters['suku'] === $suku)>{{ $suku }}</option>
					@endforeach
				</select>
				...
				<div class="flex items-stretch gap-3">
					...
					<a href="#" class="h-11 px-5 rounded-xl bg-[#173d2c] text-white text-sm font-semibold flex items-center justify-center hover:bg-[#21503a]">
					...
					<div class="p-5">
						<div class="flex items-start justify-between gap-4">
							...
							<div class="flex items-center gap-2 text-[#6f7f72]">
								<button type="button" class="w-8 h-8 rounded-full hover:bg-[#f4f7f2]"><i class="bi bi-eye"></i></button>
								<button type="button" class="w-8 h-8 rounded-full hover:bg-[#f4f7f2]"><i class="bi bi-pencil"></i></button>
								<button type="button" class="w-8 h-8 rounded-full hover:bg-[#f9eceb] text-[#c54d4d]"><i class="bi bi-trash3"></i></button>
							</div>
						</div>
